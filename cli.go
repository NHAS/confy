...
func loadCli[T any](o options, result *T) (err error) {
	...
	for _, field := range getFields(true, result) {
		if field.value.CanAddr() && field.value.CanInterface() {
			...
			flagName := strings.Join(resolvedPath, o.cli.delimiter)
			...
			if !ok {
				...
				description = fmt.Sprintf("A %s value, %s (%s)", typeName, strings.Join(field.path, o.cli.delimiter), strings.Join(resolvedPath, o.cli.delimiter))
			}
